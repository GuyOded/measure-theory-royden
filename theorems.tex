\documentclass[11pt]{article}
\usepackage[utf8]{inputenc}
\usepackage[T1]{fontenc}
\usepackage{lmodern}
\usepackage{amsmath,amssymb,amsthm}
\usepackage{enumitem}

\title{Theorems}
\author{Author Name}
\date{\today}

\newtheorem{theorem}{Theorem}[section]
\newtheorem{lemma}[theorem]{Lemma}
\newtheorem{proposition}[theorem]{Proposition}
\newtheorem*{definition*}{Definition}
\newtheorem*{proposition*}{Proposition}

\begin{document}
\maketitle

\section[1]{Preliminaries}
\setcounter{subsection}{3}
\subsection{Open Sets, Closed Sets and Borel Sets in $\mathbb{R}$}
\begin{definition*}[Sup and Inf of a Sequence of Sets]
    Let ${\{ A_n \}}_{n=1}^{\infty}$ be a countable sequence of sets that belong to a $\sigma$-algebra $\mathcal{A}$.
    Then we define the following sets that belong to $\mathcal{A}$:
    \[
        \limsup{\{ A_n \}}_{n=1}^{\infty} = \bigcap_{k=1}^\infty \bigg[\bigcup_{n=k}^\infty A_n \bigg] \; \text{and} \;
        \liminf{\{ A_n \}}_{n=1}^{\infty} = \bigcup_{k=1}^\infty \bigg[\bigcap_{n=k}^\infty A_n \bigg]
    \]
\end{definition*}

\begin{definition*}[$\sigma$-algebra]
    Given a set $X$ a collection of sets $\mathcal{A}$ is called a $\sigma$-algebra given:
    \begin{enumerate}[label=(\roman*), leftmargin=*]
        \item $\emptyset \in \mathcal{A}$
        \item $\mathcal{A}$ is closed to complements
        \item $\mathcal{A}$ is closed to a countable union of sets
    \end{enumerate}
\end{definition*}

\begin{definition*}[Borel Sigma Algebra]
    A $\sigma$-algebra will be called a Borel $\sigma$ -algebra if it is the smallest sigma algebra, containing all the of the open sets. \\
    In this case, a Borel set will refer to any set of a Borel $\sigma$-algebra.
\end{definition*}

\begin{definition*}[$G_\delta$ set]
    Any set that is a \textbf{countable intersection of open sets} is called a $G_\delta$ set.
\end{definition*}

\begin{definition*}[$F_\sigma$ set]
    Any set that is a \textbf{countable union of closed sets} is called a $F_\sigma$ set.
\end{definition*}

\subsection{Sequences of Real Numbers}
\setcounter{theorem}{14}
\begin{theorem}[The Monotone Convergence Theorem]
    A monotone sequence of real numbers converges if and only if it is bounded.
\end{theorem}

\begin{theorem}[The Bolzano-Weierstrass Theorem]
    Every bounded sequence of real numbers has a convergent subsequence.
\end{theorem}

\begin{theorem}[The Cauchy Convergence Criterion for Real Sequences]
\end{theorem}

\begin{definition*}[Limit-Superior and Limit-Inferior]
    Given a sequence ${(a_n)}_n$ we define:
    \[
        \limsup a_n = \lim_{n \to \infty} [ \sup {\{a_k : k \geq n\}} ]
    \]
    and:
    \[
        \liminf a_n = \lim_{n \to \infty} [ \inf {\{a_k : k \geq n\}} ]
    \]
\end{definition*}

\begin{enumerate}[label= (\roman*), leftmargin=*]
    \item $\limsup a_n = \ell \in \mathbb{R}$ if and only if for all $\epsilon > 0$, $a_n >  \ell - \epsilon$ for infinitely many indices and $a_n > \ell + \epsilon$ for finitely many indices.
    \item $\limsup a_n = \infty$ iff ${(a_n)}_n$ is not bounded above.
    \item \[
              \limsup{\{a_n\}} = -\liminf{\{-a_n\}}
          \]
    \item A sequence of real numbers $(a_n)$ converges to an extended real number $a$ iff
          \[
              \limsup{\{a_n\}} = \liminf{\{a_n\}} = a
          \]
    \item If $a_n \leq b_n$ for all $n$,
          \[
              \limsup {\{a_n\}} \leq \liminf {\{b_n\}}
          \]
\end{enumerate}

\section[2]{Lebesgue Measure}
\subsection[2.2]{The Lebesgue Outer Measure}
\begin{definition*}[The Lebegue Outer Measure]
    \begin{enumerate}
        \item For each interval $I \subseteq \mathbb{R}$ we define $\ell(I)$ to be its length.
        \item For each set $A \subseteq \mathbb{R}$ we define a number:
              \[
                  m^*(A) = \inf \bigg\{\sum_{i=1}^{\infty} I_i \bigg| A \subseteq \bigcup_{i=1}^\infty I_i  \bigg\}
              \]
    \end{enumerate}
\end{definition*}
It follows immediately that $m^*$ is monotone w.r.t inclusion of sets, and that $m^*(\emptyset) = 0$.

\begin{proposition*}
    The outer measure of an interval is its length
\end{proposition*}
\begin{proposition*}
    The outer measure is translation invariant
\end{proposition*}
\begin{proposition*}
    The outer measure is countably sub-additive
\end{proposition*}
With these three propositions we're ready to find a $\sigma$-algebra for this outer measure becomes a measure.
\subsection[2.3]{The $\sigma$-algebra of Lebesgue Measurable Sets}
\begin{definition*}[Lebesgue Measurable Set]
    A set $E \subseteq \mathbb{R}$ is called Lebesgue measurable if for every set $A \subseteq \mathbb{R}$ we have:
    \[
        m^*(A) = m^*(E \cap A) + m^*(E \cap A^c)
    \]
\end{definition*}

Reminder: To prove that $E$ is measurable, it suffices to show that for every set $A$ we have:
\[
    m^*(A) \geq m^*(E \cap A) + m^*(E \cap A^c)
\]
(Remember that the opposite inequality always holds by sub-additivity of $m^*$)

\setcounter{theorem}{3}
\begin{proposition}
    Any set of outer measure zero is measurable.
\end{proposition}

\begin{proposition}
    The union of a finite collection of measurable sets is measurable.
\end{proposition}

\begin{proposition}
    Let $A$ be any set and $\{E_k\}_{k=1}^n$ a finite collection of measurable sets. Then:
    \[
        m^*\Bigg( A \cap \Bigg[\bigcup_{k=1}^n E_k\Bigg] \Bigg) = \sum_{k=1}^n m^*(A \cap E_k)
    \]

    And in particular:
    \[
        m^*\Bigg(\bigcup_{k=1}^n E_k \Bigg) = \sum_{k=1}^n m^*(E_k)
    \]
\end{proposition}

\begin{proposition}
    The union of a countable collection of measurable sets is measurable.
\end{proposition}

\end{document}
